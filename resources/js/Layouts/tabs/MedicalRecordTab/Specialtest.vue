<template>
  <div class="bg-gray-100 p-4 rounded-lg">
    <h3 class="text-lg font-semibold mb-2">Special Test Records</h3>
    <UseTemplate>
    ...
    <Dialog v-if="isDesktop" v-model:open="isOpen">
      ...
      <DialogContent class="sm:max-w-[425px]">
        <DialogHeader>
          <DialogTitle>{{ isEditing ? 'Edit' : 'Add' }} Special Test Record</DialogTitle>
          <DialogDescription>
            Provide special test details and upload an image.
          </DialogDescription>
        </DialogHeader>
        <GridForm />
      </DialogContent>
    ...
    <Drawer v-else v-model:open="isOpen">
      ...
      <DrawerContent>
        <DrawerHeader class="text-left">
          <DrawerTitle>{{ isEditing ? 'Edit' : 'Add' }} Special Test Record</DrawerTitle>
          <DrawerDescription>
            Provide special test details and upload an image.
          </DrawerDescription>
        </DrawerHeader>
    ...
    <div class="overflow-x-auto mt-4 border rounded-lg">
      <Table>
        ...
        <TableBody>
          <TableRow v-if="!patient.specialtests || patient.specialtests.length === 0">
            <TableCell colspan="5" class="text-center text-gray-500">
              No special test records found.
            </TableCell>
          </TableRow>

          <TableRow v-for="record in patient.specialtests" :key="record.id">
      ...
      <AlertDialogContent>
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>
            This action cannot be undone. It will permanently delete this special test record.
          </AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
...
const addRecord = () => {
  form.post(route('specialtest.store', props.patient.id), {
    preserveScroll: true,
    onSuccess: () => {
      closeModal();
    },
...
const updateRecord = () => {
  ...
  router.post(route('specialtest.update', { id: selectedRecordId.value }), formData, {
    preserveScroll: true,
    onSuccess: () => {
      closeModal();
    },
...
const handleDelete = () => {
  form.delete(route('specialtest.destroy', recordToDeleteId.value), {
    preserveScroll: true,
    onSuccess: () => {
