  ...
  <div class="bg-gray-100 p-4 rounded-lg">
    ...
    <Dialog v-if="isDesktop" v-model:open="isOpen">
      ...
      <DialogContent class="sm:max-w-[425px]">
        <DialogHeader>
          <DialogTitle>{{ isEditing ? 'Edit' : 'Add' }} Microbiology Record</DialogTitle>
          <DialogDescription>
            Provide microbiology test details and upload an image.
          </DialogDescription>
        </DialogHeader>
        <GridForm />
      </DialogContent>
    ...
    <Drawer v-else v-model:open="isOpen">
      ...
      <DrawerContent>
        <DrawerHeader class="text-left">
          <DrawerTitle>{{ isEditing ? 'Edit' : 'Add' }} Microbiology Record</DrawerTitle>
          <DrawerDescription>
            Provide microbiology test details and upload an image.
          </DrawerDescription>
        </DrawerHeader>
    ...
    <div class="overflow-x-auto mt-4 border rounded-lg">
      <Table>
        ...
        <TableBody>
          <TableRow v-if="!patient.microbiologies || patient.microbiologies.length === 0">
            <TableCell colspan="5" class="text-center text-gray-500">
              No microbiology records found.
            </TableCell>
          </TableRow>

          <TableRow v-for="record in patient.microbiologies" :key="record.id">
      ...
      <AlertDialogContent>
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>
            This action cannot be undone. This will permanently delete the selected microbiology record.
          </AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
...
const handleDelete = () => {
  form.delete(route('microbiology.destroy', recordToDeleteId.value), {
    preserveScroll: true,
    onSuccess: () => {
