<template>
  <div class="bg-gray-100 p-4 rounded-lg">
    <h3 class="text-lg font-semibold mb-2">Histopath Records</h3>
    <UseTemplate>
    ...
    <Dialog v-if="isDesktop" v-model:open="isOpen">
      ...
      <DialogContent class="sm:max-w-[425px]">
        <DialogHeader>
          <DialogTitle>{{ isEditing ? 'Edit' : 'Add' }} Histopath Record</DialogTitle>
          <DialogDescription>
            Provide histopathology details and upload an image if needed.
          </DialogDescription>
        </DialogHeader>
        <GridForm />
      </DialogContent>
    ...
    <Drawer v-else v-model:open="isOpen">
      ...
      <DrawerContent>
        <DrawerHeader class="text-left">
          <DrawerTitle>{{ isEditing ? 'Edit' : 'Add' }} Histopath Record</DrawerTitle>
          <DrawerDescription>
            Provide histopathology details and upload an image if needed.
          </DrawerDescription>
        </DrawerHeader>
    ...
    <div class="overflow-x-auto mt-4 border rounded-lg">
      <Table>
        ...
        <TableBody>
          <TableRow v-if="!patient.histopaths || patient.histopaths.length === 0">
            <TableCell colspan="5" class="text-center text-gray-500">
              No histopathology records found.
            </TableCell>
          </TableRow>

          <TableRow v-for="record in patient.histopaths" :key="record.id">
      ...
      <AlertDialogContent>
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>
            This action cannot be undone. It will permanently delete this histopathology record.
          </AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
...
const addRecord = () => {
  form.post(route('histopath.store', props.patient.id), {
    preserveScroll: true,
    onSuccess: () => {
      closeModal();
    },
...
const updateRecord = () => {
  ...
  router.post(route('histopath.update', { id: selectedRecordId.value }), formData, {
    preserveScroll: true,
    onSuccess: () => {
      closeModal();
    },
...
const handleDelete = () => {
  form.delete(route('histopath.destroy', recordToDeleteId.value), {
    preserveScroll: true,
    onSuccess: () => {
