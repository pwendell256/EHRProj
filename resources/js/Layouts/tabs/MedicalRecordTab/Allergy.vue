    ...
    <div class="bg-gray-100 p-4 rounded-lg">
        ...
        <table class="w-full border-collapse border border-gray-300">
          <thead>
            <tr class="bg-gray-800 text-white">
              <th class="p-2 border border-gray-300">Allergy</th>
          ...
          <tbody>
            <tr class="bg-white border border-gray-300" v-for="record in patient.allergies" :key="record.id">
              <td class="p-2 border border-gray-300">{{ record.allergy }}</td>
              <td class="p-2 border border-gray-300">{{ record.dateTime }}</td>
              <td class="p-2 border border-gray-300">{{ record.description }}</td>
      ...
      <div v-if="showModal" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
        <div class="bg-white p-6 rounded-lg shadow-lg w-96">
          <h3 class="text-lg font-semibold mb-4">{{ isEditing ? 'Edit' : 'Add' }} Allergy Record</h3>
          <form @submit.prevent="isEditing ? updateRecord() : addRecord()">
            <div class="mb-2">
              <label class="block text-sm font-medium">Allergy</label>
              <input v-model="form.allergy" class="w-full p-2 border rounded" required />
            </div>
            <div class="mb-2">
            ...
            <div class="mb-2">
              ...
              <input v-model="form.description" class="w-full p-2 border rounded" required />
            </div>
            <div class="mb-4">
...
const updateRecord = () => {
  ...
  const deleteRecord = (id) => {
    if (confirm('Are you sure you want to delete this record?')) {
      form.delete(route('allergy.destroy', id), {
        preserveScroll: true,
      });
    }
  };
